...
const audio = {
    ...
    init() {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                this.eveningVolume = new Tone.Volume(-Infinity).connect(state.volume);
                this.nightVolume = new Tone.Volume(-Infinity).connect(state.volume);

                const eveningPlayerPromise = new Promise((res, rej) => {
                    this.eveningPlayer = new Tone.Player({
                        url: "https://files.catbox.moe/cjowme.mp3",
                        loop: true,
                        onload: res,
                        onerror: rej,
                    }).connect(this.eveningVolume);
                });
                ...
                await Promise.all([eveningPlayerPromise, nightPlayerPromise, samplerPromise]);
    ...
    setAmbientTrack(theme) {
        ...
        if (theme === 'evening') {
            if (this.eveningPlayer.state !== 'started') this.eveningPlayer.start();
            this.eveningVolume.volume.rampTo(targetVolume, fadeDuration);
            ...
            this.eveningVolume.volume.rampTo(-Infinity, fadeDuration);
        } else { // It's 'day' theme
            this.eveningVolume.volume.rampTo(-Infinity, fadeDuration);
            this.nightVolume.volume.rampTo(-Infinity, fadeDuration);
        }
    ...
    startPlayback() {
        ...
        Tone.Transport.scheduleOnce(() => {
            ...
            if (bodyClass.includes('theme-evening') && this.eveningPlayer.state === 'started') {
                this.eveningVolume.volume.rampTo(-16, 1.5);
            } else if (bodyClass.includes('theme-night') && this.nightPlayer.state === 'started') {
...
const experience = {
    ...
    setupBackgrounds() {
        ...
        // --- Create Sunset Scene ---
        const sunsetScene = document.createElement('div');
        sunsetScene.className = 'background-scene sunset-scene';

        const sun = document.createElement('div');
        sun.className = 'sun';
        sunsetScene.appendChild(sun);

        const ocean = document.createElement('div');
        ocean.className = 'ocean';

        // --- Create realistic SVG reflection ---
        const reflectionSVG = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
        reflectionSVG.setAttribute('class', 'reflection');
        reflectionSVG.setAttribute('viewBox', '0 0 300 300');
        reflectionSVG.setAttribute('preserveAspectRatio', 'none');

        const defs = document.createElementNS('http://www.w3.org/2000/svg', 'defs');

        // Define the gradient for the reflection
        const gradient = document.createElementNS('http://www.w3.org/2000/svg', 'linearGradient');
        gradient.setAttribute('id', 'reflection-gradient');
        gradient.setAttribute('gradientTransform', 'rotate(90)');
        gradient.innerHTML = `
            <stop offset="0%" stop-color="var(--accent1)" stop-opacity="0.8" />
            <stop offset="70%" stop-color="var(--accent1)" stop-opacity="0" />
        `;
        defs.appendChild(gradient);

        // Define the turbulence filter for the shimmer effect
        const filter = document.createElementNS('http://www.w3.org/2000/svg', 'filter');
        filter.setAttribute('id', 'shimmer-filter');
        filter.innerHTML = `
            <feTurbulence type="fractalNoise" baseFrequency="0.01 0.04" numOctaves="2" result="turbulence" seed="10" />
            <feDisplacementMap in="SourceGraphic" in2="turbulence" scale="20" xChannelSelector="R" yChannelSelector="G" />
        `;
        defs.appendChild(filter);
        reflectionSVG.appendChild(defs);

        // Create the shape that will be distorted
        const reflectionPath = document.createElementNS('http://www.w3.org/2000/svg', 'rect');
        reflectionPath.setAttribute('width', '300');
        reflectionPath.setAttribute('height', '300');
        reflectionPath.setAttribute('fill', 'url(#reflection-gradient)');
        reflectionPath.setAttribute('filter', 'url(#shimmer-filter)');
        reflectionSVG.appendChild(reflectionPath);

        ocean.appendChild(reflectionSVG);

        // Animate the turbulence filter with GSAP for a continuous shimmer
        gsap.to(filter.querySelector('feTurbulence'), {
        ...
        // New SVG-based waves for a more organic look
        const oceanWavesSVG = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
        oceanWavesSVG.setAttribute('class', 'ocean-waves');
        oceanWavesSVG.setAttribute('viewBox', '0 0 1440 100');
        oceanWavesSVG.setAttribute('preserveAspectRatio', 'xMidYMax slice');
        
        const waveGroup = document.createElementNS('http://www.w3.org/2000/svg', 'g');
        // Color and opacity are now controlled via CSS for better theme integration
        
        const wavePaths = [
            "M0,50 C300,80 500,20 800,50 C1100,80 1300,20 1440,50 V100 H0 Z",
            "M0,60 C250,100 450,40 750,60 C1050,80 1250,30 1440,60 V100 H0 Z",
            "M0,70 C200,50 400,90 700,70 C1000,50 1200,90 1440,70 V100 H0 Z"
        ];
        
        wavePaths.forEach(pathData => {
            const path = document.createElementNS('http://www.w3.org/2000/svg', 'path');
            path.setAttribute('d', pathData);
            waveGroup.appendChild(path);
        });

        oceanWavesSVG.appendChild(waveGroup);
        ocean.appendChild(oceanWavesSVG);

        sunsetScene.appendChild(ocean);
        DOM.backgroundElements.appendChild(sunsetScene);
